...
export default function Header({ onSignOut, userEmail }) {
  ...
  return (
    <header className="header">
      ...
      <Switch>
        <Route exact path="/">
          <div className={`header__user-info-box ${isOpenedMenu ? "header__user-info-box_opened" : ""}`}>
          ...
          <button
            type="button"
            className={`burger-btn header__burger-btn ${isOpenedMenu ? "burger-btn_opened-menu" : ""}`}
            aria-label="Кнопка меню."
            onClick={handlerMenuBtnClick}
          >
            <div className="burger-btn__cross"></div>
          </button>
        </Route>
        <Route path="/sign-in">
